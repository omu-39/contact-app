...
<body class="min-h-screen bg-[#f5efe7] text-gray-700">
    <x-header>
        <x-slot name="right">
            <a href="{{ route('register') }}" class="inline-flex items-center rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-200">
                register
            </a>
        </x-slot>
    </x-header>
    ...
    <main class="flex min-h-[calc(100vh-80px)] items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
        <div class="w-full max-w-md">
            <h1 class="text-center text-3xl font-semibold tracking-[0.12em] text-stone-500 font-serif">Login</h1>
            ...
            <div class="mt-10 rounded-[32px] bg-white/95 p-10 shadow-xl shadow-gray-300/20 ring-1 ring-gray-200">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" value="{{ old('email') }}" placeholder="例: test@example.com" required autofocus class="mt-3 w-full rounded-md border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-200" />
                    </div>
